# =================================================================================
#  CAPTURA DE LINKS DE VÍDEO - v5.4 (Gerenciamento Automático do ChromeDriver)
...
# --- Função Inteligente para Determinar o Caminho Base ---
def get_base_path():
...
class AboutDialog(ctk.CTkToplevel):
    def __init__(self, master=None):
        ...
        label_link = ctk.CTkLabel(link_frame, text=github_url, font=link_font, text_color="#1E90FF")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.title("Capturador de Links v5.4") # Versão atualizada
    ...
    def find_episode_links_heuristic(self, soup, base_url):
        ...
        padrao_texto_ep = re.compile(r'(ep(is[oó]dio)?|cap(ítulo)?|parte|part|ova|ona|filme|movie|especial|special)\s*[\d.\-]+', re.IGNORECASE)
        seletores_container = ['.episodes-list', '.lista-de-episodios', '.episode_list', '.episodios', '#episode-list', '.video-list', 'ul[class*="episode"]', 'div[class*="episode"]']
        search_area = soup
    ...
    def scraping_worker(self, url):
        # =========================================================================
        #  AQUI ESTÁ A LÓGICA DE SCRAPING ATUALIZADA
        # =========================================================================
        service = None
        driver = None
        serie_title = "Série"
        try:
            service = Service() # Selenium Manager cuidará do chromedriver
            options = Options()
            ...
            try:
                title_tag = soup.find(['h1', 'h2'], class_=['entry-title', 'post-title', 'title']) or soup.find('h1')
                serie_title = title_tag.get_text(strip=True) if title_tag else (soup.title.string.strip() if soup.title else "Série Desconhecida")
            ...
            with open(NOME_ARQUIVO_SAIDA, 'a', encoding='utf-8') as f: f.write(header)
            ...
            for i, link_ep in enumerate(links_dos_episodios):
                if self.cancel_event.is_set():
                    self.update_queue.put(("finished", "Cancelado: Processamento interrompido."))
                    break
